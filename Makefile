...
bin/kyverno-agent-tools-linux-amd64:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o bin/kyverno-agent-tools-amd64 ./cmd

bin/kyverno-agent-tools-linux-amd64.sha256: bin/kyverno-agent-tools-amd64
...
.PHONY: build
build: $(LOCALBIN) clean bin/kyverno-agent-tools-amd64.sha256 bin/kyverno-agent-tools-arm64.sha256 bin/kyverno-agent-tools-darwin-amd64.sha256 bin/kyverno-agent-tools-darwin-arm64.sha256
build:
